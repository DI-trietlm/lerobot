...
[`reports/diagnosis_presentation.html`](reports/diagnosis_presentation.html) (open in a browser, offline).
...
- `notebooks/` — reproducible offline probes and reports.
- `scripts/` — reusable data/model diagnosis utilities.
- `reports/` — rendered notes, exported HTML, and markdown findings.
- `attention/` — attention and occlusion explainability utilities.
- `artifacts/` — ignored local outputs generated by scripts/notebooks.
...
|---|----------|---------------------|--------|
| 1 | `notebooks/REPORT-pouring-startpose-and-phases.ipynb` | Start-pose IQR + grasp/pour phases | start-pose OOD fixed; not the cause |
| 2 | `notebooks/openloop_replay_smolvla.ipynb` | Does the **model** reproduce training actions? | yes (MAE ~2°); model+norm fine; image-blind |
| 3 | `notebooks/deploy_image_probe_smolvla.ipynb` | Do **deploy images** make it collapse? | no — RGB/BGR, 256-resize, JPEG all cleared |
| 4 | `notebooks/deploy_state_replay_smolvla.ipynb` | Image or state? | state-dominated; `Δ≈0` reproduces standstill |
